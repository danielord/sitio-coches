...
// .wrangler/tmp/bundle-Qobz9m/checked-fetch.js
...
// workers/node_modules/hono/dist/utils/cookie.js
...
// workers/node_modules/hono/dist/utils/stream.js
...
// workers/node_modules/hono/dist/http-exception.js
...
// workers/api/coches.ts
var coches = new Hono2();
coches.get("/", async (c) => {
  try {
    ...
    const cochesFormatted = results.map((row) => ({
      ...row,
...
coches.post("/", async (c) => {
  try {
    ...
    await c.env.DB.prepare(`
      INSERT INTO coches (id, marca, modelo, a\xF1o, precio, kilometraje, combustible, transmision, color, descripcion, imagenes, vendedorId)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      id,
      body.marca,
...
// workers/api/vendedores.ts
var vendedores = new Hono2();
...
// workers/node_modules/hono/dist/utils/jwt/jwt.js
var jwt_exports = {};
...
// workers/node_modules/hono/dist/utils/encode.js
...
// workers/node_modules/hono/dist/utils/jwt/types.js
...
// workers/node_modules/hono/dist/middleware/jwt/index.js
...
auth.post("/registro", async (c) => {
  try {
    ...
    if (existing.length > 0) {
      return c.json({ message: "El email ya est\xE1 registrado" }, 400);
    }
    const id = crypto.randomUUID();
    await c.env.DB.prepare(`
      INSERT INTO vendedores (id, nombre, email, telefono, password)
...
ai.post("/descripcion", async (c) => {
  try {
    const body = await c.req.json();
    const prompt = `Genera una descripci\xF3n atractiva y profesional para un coche en venta con estas caracter\xEDsticas:
...
Transmisi\xF3n: ${body.transmision}
Color: ${body.color}

La descripci\xF3n debe ser en espa\xF1ol, de 2-3 p\xE1rrafos, destacando las caracter\xEDsticas principales y beneficios del veh\xEDculo.`;
    ...
    const descripcion = data.choices?.[0]?.message?.content || `${body.marca} ${body.modelo} ${body.a\u00F1o} en excelente estado. Veh\xEDculo con ${body.kilometraje} km, motor ${body.combustible} y transmisi\xF3n ${body.transmision}. Color ${body.color}.`;
    return c.json({ descripcion });
  } catch (error) {
    return c.json({ error: "Error generando descripci\xF3n" }, 500);
  }
});
    ...
    const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/YOUR_ACCOUNT_ID/ai/run/@cf/meta/llama-2-7b-chat-int8`, {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: "system",
            content: 'Analiza si el siguiente texto contiene contenido inapropiado, spam, lenguaje ofensivo o informaci\xF3n falsa. Responde solo con "APROPIADO" o "INAPROPIADO".'
          },
          {
            role: "user",
            content: texto
...
upload.post("/", async (c) => {
  try {
    ...
    if (!file) {
      return c.json({ error: "No se proporcion\xF3 archivo" }, 400);
    }
...
// workers/api/valoraciones.ts
var valoraciones = new Hono2();
...
valoraciones.post("/", async (c) => {
  try {
    ...
    if (!puntuacion || puntuacion < 1 || puntuacion > 5) {
      return c.json({ error: "Puntuaci\xF3n debe ser entre 1 y 5" }, 400);
    }
    const id = crypto.randomUUID();
    await c.env.DB.prepare(`
...
// .wrangler/tmp/bundle-Qobz9m/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Qobz9m/middleware-loader.entry.ts
